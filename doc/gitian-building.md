...
*Setup instructions for a gitian build of Rubika using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Rubika
...
- [Building Rubika](#building-rubika)
...
Clone the git repositories for rubika and gitian and then checkout the rubika version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/rubika-crypto/rubika.git
cd rubika
...
Follow the instructions in [doc/release-process.md](release-process.md) in the rubika repository
...
To build Rubika (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the rubika repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/rubika/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/rubika-crypto/rubika
...
For example:
```bash
URL=https://github.com/crowning-/rubika.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit rubika=${COMMIT} --url rubika=${URL} ../Rubika/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit rubika=${COMMIT} --url rubika=${URL} ../Rubika/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit rubika=${COMMIT} --url rubika=${URL} ../Rubika/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/rubika-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/rubika-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/rubika-build.assert
```
...
[rubika/gitian.sigs](https://github.com/rubika-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
